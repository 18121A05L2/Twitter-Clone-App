...
error SmartWallet_InsufficientFunds();
error SmartWallet_InvalidEntryPoint();
error SmartWallet_InvalidEntryPointOrOwner();
...
contract SmartWallet is IAccount, Ownable {
    ...
    modifier onlyEntryPoint() {
        if (msg.sender != address(i_entryPoint)) revert SmartWallet_InvalidEntryPoint();
        _;
    }

    modifier requireFromEntryPointOrOwner() {
        if (msg.sender != address(i_entryPoint) && msg.sender != owner()) {
            revert SmartWallet_InvalidEntryPointOrOwner();
        }
        _;
    }
    ...
    function _payPrefund(uint256 missingAccountFunds) internal {
        ...
        if (!success) revert SmartWallet_InsufficientFunds();
    }
